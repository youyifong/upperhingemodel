...
#setwd("/Users/adamelder/Desktop/RAfiles/")'
library(tidyverse)
library(kableExtra)
...
get_line <- function(var_type, ss, param){
  ...
  if (var_type %in% c(1, 3)){
    ...
    all_res <- c(paste0(seg_sum[1], " (", seg_sum[2], ")"), 
                 paste0(seg_sum[5]), paste0(seg_sum[3]),
                 paste0(uph_sum[1], " (", uph_sum[2], ")"), 
    ...
    for(jj in 1:length(ve_names)){
      ...
      sum_data <- round(est_mbsc(sub_data, type = tp, tr_v = trv, 
                        ...
                        c(2, 0, 2, 2, 3, 2))
      if (jj == 1){
        all_res <- c(paste0(sum_data[1], " (", sum_data[2], ")"), 
                     paste0(sum_data[3]),
                     paste0(sum_data[5], " (", sum_data[4], ")"))
        names(all_res) <- c("Est (% Bias)", "MC SD", paste0(ve_names[jj], " Cov (SE)"))
      } else {
        new_obs <- paste0(sum_data[5], " (", sum_data[4], ")")
        names(new_obs) <- paste0(ve_names[jj], " Cov (SE)")
...
res_1 <- mk_tbl(c(50, 100, 500), c("Beta_z", "Beta", "Cpt"), 1)
row.names(res_1) <- NULL
colnames(res_1) <- c("n", rep(c(" Est(%Bias)", " Coverage", " MC SD"), times = 2), 
                     "SE Ratio", "IQR Ratio")

knitr::kable(res_1, format = "latex", booktabs = TRUE) %>% 
...
colnames(res_2) <- c("n", "Est (% Bias)", "MC SD", "Bootstrap Coverage (SE)",
                     "Model based Coverage (SE)")

knitr::kable(res_2, format = "latex", booktabs = TRUE) %>% 
  kable_styling(latex_options = c("striped", "scale_down")) %>%
...
colnames(res_4) <- c("n", "Est (% Bias)", "MC SD", "Bootstrap Coverage (SE)",
                     "Model based Coverage (SE)")

knitr::kable(res_4, format = "latex", booktabs = TRUE) %>% 
  kable_styling(latex_options = c("striped", "scale_down")) %>%
  add_header_above(c(" ", "Segmented Model" = 3, "Upper Hinge Model" = 3, " " = 2)) %>%
  group_rows("Beta_Z", 1, 3) %>% group_rows("Beta", 4, 6) %>% 
  group_rows("Chpt", 7, 9)
